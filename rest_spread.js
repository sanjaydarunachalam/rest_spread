...
keys and values of the first object and second*/ 
...
const doubleAndReturnArgs = (arr, ...args) => [...arr, ...args.map(dblArgs => dblArgs*2)];
...
function removeRandom(items) {
    const removeRdm = items =>{
        let indexRdm = Math.floor(Math.random()* items.length);
        return [...items.slice(0,indexRdm), ...items.slice(indexRdm +1)];
    }
    return removeRdm; 
}
